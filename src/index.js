...
// Number-cake sizing — ONE size for every digit count (each digit stands `height` tall; the board just
// grows wider). Exported so the Cake Shape Studio authors against the SAME default + resolver the designer
// sizes with (one source, no drift), and previews the true footprint.
export { NUMBER_SIZE_DEFAULT, numberDigitCount, numberSize, numberTierDims } from './designer/geometry/numberShape.js';
