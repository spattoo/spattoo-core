...
describe('what the rows open', () => {
  ...
  it('reuses the existing credits panel', () => {
    expect(panel).toMatch(/import BuyCreditsPanel from '\.\.\/billing\/BuyCreditsPanel\.jsx'/);
  });
  ...
  it('opens a screen from both rows', () => {
    expect(panel).toMatch(/onOpenSmartTools=\{\(\) => setCreditsOpen\(true\)\}/);
    expect(panel).toMatch(/onOpenMessages=\{\(\) => setMessagesOpen\(true\)\}/);
  });
  ...
  it('leaves settings open behind them', () => {
    expect(panelCode).not.toMatch(/setCreditsOpen\(true\);\s*onClose/);
    expect(panelCode).not.toMatch(/onOpenMessages=\{\(\) => \{[^}]*onClose/);
  });
...
describe('it left the store settings', () => {
  it('no longer renders the section inline among them', () => {
    expect(panel).toMatch(/<Section title="Top-ups">/);
