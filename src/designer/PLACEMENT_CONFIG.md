...
```jsonc
{
  ...
  // ── Print finish (2D image only, §1) — flat AND relief stickers; absent → renderer defaults (1.12 / 0.22) ──
  "print_finish": { "saturation": 1.12, "emissive": 0.22 },  // albedo chroma pre-boost + decal self-illumination
...
```jsonc
{
  ...
  "placement_config": {                                   // HOW it behaves — the FULL superset (= §0).
    ...
    "relief":  { "lift": 0.07, "normalScale": 0.8, "roughness": 0.95, "sheen": 0, "envIntensity": 0.4, "toneMapped": false,
                 "bake": { "puff": 0.5, "domeBlur": 34, "edgeRound": 16, "detail": 0.4, "grain": 0.5, "delit": 0, "flipY": false, "flattenThin": 0 } },  // raised fondant cut-out (admin Relief Studio); flattenThin>0 keeps thin protrusions flush; optional sibling "flatMask": "data:image/png;base64,…" (authored ~128px paint-mask, black=flush)
    ...
    "print_finish": { "saturation": 1.12, "emissive": 0.22 },  // 2D image print vividness (flat + relief); absent → 1.12/0.22
