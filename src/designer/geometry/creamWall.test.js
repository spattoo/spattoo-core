...
const TAU = Math.PI * 2;
const PIPED = { relief: 0.06, ropes: 30, round: 0.45, vary: 0.28, wobble: 0.10, teeth: 4, teethDepth: 0.18 };
...
describe('makeRopeField — separate strokes around the wall', () => {
  ...
  it('gives neighbouring strokes different widths — identical ones read as machined', () => {
    ...
    const even = makeRopeField({ ...PIPED, vary: 0 });
    const hand = makeRopeField(PIPED);
    const at = (f) => Array.from({ length: PIPED.ropes }, (_, k) => f(k / PIPED.ropes));
  ...
  it('flattens with vary 0 — the knob is what makes the hand show', () => {
    const f = makeRopeField({ ...PIPED, vary: 0 });
    expect(f(0.5 / 30)).toBeCloseTo(f(0.5 / 30 + 1 / 30), 10);
  });
});
...
describe('buildStyledTop', () => {
  ...
  it('reaches the wall it caps — the rim covers the crest at every angle, or daylight shows', () => {
    ...
    // The rim is the widest vertex at each angle; compare it with the wall's crest line there.
    const worst = new Map();
    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i), z = pos.getZ(i);
      const r = Math.hypot(x, z);
      const key = Math.round(Math.atan2(z, x) * 100);
      if (!(worst.get(key) >= r)) worst.set(key, r);
    }
    for (const [key, r] of worst) {
      const theta = key / 100;
      expect(r).toBeGreaterThanOrEqual(1 + Math.max(0, crest(theta, 1)) - 1e-6);
    }
  });
