...
export default function PreviewTile({ checked, onToggle, label, height = 104, locked = false, children }) {
  ...
  return (
    <div>
      <div style={{ position: 'relative', width: '100%', height, borderRadius: 10, overflow: 'hidden', border: `1.5px solid ${checked ? INK : '#cdccd3'}`, background: '#cfcdd6' }}>
        {children}
        <label title={frozen ? 'This decoration can’t be removed' : checked ? 'Remove from cake' : 'Add to cake'} onPointerDown={e => e.stopPropagation()}
          ...
          <input type="checkbox" checked={checked} disabled={frozen} onChange={onToggle} style={{ accentColor: INK, width: 15, height: 15, cursor: frozen ? 'not-allowed' : 'pointer', margin: 0 }} />
        </label>
      </div>
      {label && (
        <span style={{ display: 'block', marginTop: 7, fontSize: 11, fontWeight: 700, color: INK, fontFamily: "'Quicksand',sans-serif", textTransform: 'uppercase', letterSpacing: 0.5, lineHeight: 1.25, textAlign: 'center' }}>{label}</span>
      )}
    </div>
  );
}
